...
const VisitLogReport = () => {
  ...
  const handleDownload = async () => {
    try {
      const res = await api.get(`/visitLogs/all/export-average?from=${from}&to=${to}`, { responseType: 'blob' });
      console.log(res);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(error?.response?.data?.message || error?.message || "Failed to fetch visit logs");
      }
      setLoading(false);
    };
    fetchData();
  ...
  return (
    <div style={{ padding: 24, background: theme === 'dark' ? '#181c20' : '#f7faff', minHeight: '100vh' }}>
      ...
      {/* Top Cards */}
      {data && (
        <div className="visitlog-cards-row" style={{ display: 'flex', gap: 24, marginBottom: 32, flexWrap: 'wrap' }}>
          <div className="visitlog-card" style={{
            flex: 1,
            minWidth: 220,
            ...
            alignItems: 'center',
            transition: 'transform 0.25s cubic-bezier(.4,2,.6,1), box-shadow 0.25s, background 0.18s, color 0.18s',
            color: theme === 'dark' ? '#e3eaf2' : '#1a237e',
          }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: theme === 'dark' ? '#a4c2f4' : '#3b5998', marginBottom: 8 }}>Visits</div>
            <div style={{ fontSize: 32, fontWeight: 700 }}>{data?.totalVisits ?? 0}</div>
            <div style={{ fontSize: 15, color: theme === 'dark' ? '#a4c2f4' : '#1976d2', marginTop: 4 }}>Total Visits</div>
            <div style={{ fontSize: 18, fontWeight: 600, marginTop: 12 }}>{data?.totalAverageVisits?.toFixed(2) ?? 0} <span style={{ fontWeight: 400, fontSize: 14 }}>Total Avg</span></div>
            <div style={{ fontSize: 18, fontWeight: 600, marginTop: 4 }}>{data?.averageVisit?.toFixed(2) ?? 0} <span style={{ fontWeight: 400, fontSize: 14 }}>Avg</span></div>
          </div>
          <div className="visitlog-card" style={{
            flex: 1,
            minWidth: 220,
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: theme === 'dark' ? '#f4bfa4' : '#d2691e', marginBottom: 8 }}>New Visits</div>
            <div style={{ fontSize: 32, fontWeight: 700 }}>{data?.totalNewVisit ?? 0}</div>
            <div style={{ fontSize: 15, color: theme === 'dark' ? '#f4bfa4' : '#d2691e', marginTop: 4 }}>Total New</div>
            <div style={{ fontSize: 18, fontWeight: 600, marginTop: 12 }}>{data?.averageNewVisit?.toFixed(2) ?? 0} <span style={{ fontWeight: 400, fontSize: 14 }}>Avg</span></div>
          </div>
          <div className="visitlog-card" style={{
            flex: 1,
            minWidth: 220,
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: theme === 'dark' ? '#a4f4c2' : '#388e3c', marginBottom: 8 }}>Follow Up Visits</div>
            <div style={{ fontSize: 32, fontWeight: 700 }}>{data?.totalFollowUpVisit ?? 0}</div>
            <div style={{ fontSize: 15, color: theme === 'dark' ? '#a4f4c2' : '#388e3c', marginTop: 4 }}>Total Follow Up</div>
            <div style={{ fontSize: 18, fontWeight: 600, marginTop: 12 }}>{data?.averageFollowUpVisit?.toFixed(2) ?? 0} <span style={{ fontWeight: 400, fontSize: 14 }}>Avg</span></div>
          </div>
        </div>
      )}
      {/* User Table */}
      {data && (
        <div style={{ background: theme === 'dark' ? '#23272b' : '#fafdff', borderRadius: 18, boxShadow: '0 2px 12px rgba(44,62,80,0.08)', padding: 0, overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 16, minWidth: 600 }}>
            <thead>
              <tr style={{ background: theme === 'dark' ? '#232b33' : '#e3eaf2', color: theme === 'dark' ? '#a4c2f4' : '#1a237e', fontWeight: 700 }}>
                <th style={{ padding: '16px 8px', borderTopLeftRadius: 18 }}>User</th>
                <th>Total Visits</th>
                <th>Avg Visit</th>
                <th>New Visits</th>
            ...
            <tbody>
              {data?.users?.map((user, idx, arr) => (
                <tr key={user._id} className="visitlog-row" style={{ background: theme === 'dark' ? '#23272b' : '#fafdff', borderBottom: idx < arr.length - 1 ? `1.5px solid ${theme === 'dark' ? '#232b33' : '#e3eaf2'}` : 'none', transition: 'background 0.18s, color 0.18s, box-shadow 0.18s', cursor: 'pointer' }}
                  onClick={() => handleUserRowClick(user)}>
                  <td style={{ padding: '14px 8px', fontWeight: 600, color: theme === 'dark' ? '#a4c2f4' : '#1976d2', borderBottomLeftRadius: idx === arr.length - 1 ? 18 : 0 }}>{user.username}</td>
                  <td style={{ textAlign: 'center', fontWeight: 500 }}>{user.totalVisits ?? 0}</td>
                  <td style={{ textAlign: 'center', fontWeight: 500 }}>{user.averageVisit ?? 0}</td>
                  <td style={{ textAlign: 'center', fontWeight: 500 }}>{user.totalNewVisit ?? 0}</td>
                  <td style={{ textAlign: 'center', fontWeight: 500, borderBottomRightRadius: idx === arr.length - 1 ? 18 : 0 }}>{user.totalFollowUpVisit ?? 0}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {userModal.open && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.18)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: theme === 'dark' ? '#23272b' : '#fff', borderRadius: 18, boxShadow: '0 8px 32px rgba(44,62,80,0.18)', minWidth: 340, maxWidth: 600, width: '98vw', color: theme === 'dark' ? '#fff' : '#23272b', position: 'relative', padding: '2.2rem 1.7rem 1.5rem 1.7rem', maxHeight: '90vh', overflowY: 'auto' }}>
            <button onClick={() => setUserModal({ open: false, user: null })} style={{ position: 'absolute', top: 18, right: 18, background: 'none', border: 'none', fontSize: 22, color: '#888', cursor: 'pointer', zIndex: 2 }}><FaTimes /></button>
            <div style={{ fontWeight: 700, fontSize: 22, marginBottom: 10, textAlign: 'center' }}>{userModal.user?.username}</div>
            {/* Date filter */}
            <div style={{ display: 'flex', gap: 18, marginBottom: 18, justifyContent: 'center' }}>
              <div>
                ...
                <input type="date" value={userLogFrom} onChange={e => setUserLogFrom(e.target.value)} style={{ borderRadius: 8, border: '1.5px solid #a4c2f4', padding: '8px 14px', background: theme === 'dark' ? '#181c20' : '#f7faff', color: theme === 'dark' ? '#fff' : '#23272b' }} />
              </div>
              <div>
                <label style={{ fontWeight: 500, marginBottom: 6, display: 'block' }}>To</label>
                <input type="date" value={userLogTo} onChange={e => setUserLogTo(e.target.value)} style={{ borderRadius: 8, border: '1.5px solid #a4c2f4', padding: '8px 14px', background: theme === 'dark' ? '#181c20' : '#f7faff', color: theme === 'dark' ? '#fff' : '#23272b' }} />
              </div>
              <button
                onClick={() => {
                ...
                style={{ borderRadius: 8, padding: '8px 22px', fontSize: 16, background: theme === 'dark' ? '#a4c2f4' : '#1976d2', color: theme === 'dark' ? '#23272b' : '#fff', border: 'none', fontWeight: 600, alignSelf: 'end', marginTop: 22, marginLeft: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}><FaFilter /> Apply</button>
            </div>
            {/* Loading, error, or data */}
            {userLogLoading && (
              <div style={{ margin: '40px 0', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 120 }}>
                <div className="spinner" style={{ width: 36, height: 36, border: `4px solid ${theme === 'dark' ? '#a4c2f4' : '#1976d2'}`, borderTop: '4px solid transparent', borderRadius: '50%', animation: 'spin 1s linear infinite', marginBottom: 12 }} />
                <div style={{ color: theme === 'dark' ? '#a4c2f4' : '#1976d2', fontSize: 16 }}>Loading...</div>
              </div>
            )}
            {userLogError && <div style={{ color: '#e53935', fontWeight: 500, margin: '18px 0', textAlign: 'center' }}>{userLogError}</div>}
            {userLogData && (
              <div>
                <div style={{ fontWeight: 600, fontSize: 18, marginBottom: 10, textAlign: 'center' }}>Total Visits: {userLogData.visitLogCounter ?? 0}</div>
                <div style={{ maxHeight: 320, overflowY: 'auto' }}>
                  {userLogData.visitLogs && userLogData.visitLogs.length > 0 ? (
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15, marginTop: 8 }}>
                      ...
                      <tbody>
                        {userLogData.visitLogs.map((log, i) => (
                          <tr key={log.logId || i} style={{ borderBottom: `1px solid ${theme === 'dark' ? '#232b33' : '#e3eaf2'}` }}>
                            <td style={{ padding: '8px 6px', fontWeight: 600 }}>{log.pocName}</td>
                            <td style={{ textAlign: 'center' }}>{log.visitType}</td>
                            <td style={{ textAlign: 'center' }}>{log.visitDate?.slice(0, 10)}</td>
                            <td style={{ textAlign: 'center' }}>{log.pocAddress}</td>
                    ...
                    <div style={{ color: theme === 'dark' ? '#a4c2f4' : '#1976d2', textAlign: 'center', margin: '18px 0' }}>No visit logs found for this user in the selected range.</div>
                  )}
                </div>
              </div>
            )}
